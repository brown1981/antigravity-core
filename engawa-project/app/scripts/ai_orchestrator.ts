...
async function runAnalysisLoop() {
  // 失敗が続いている場合は実行をスキップまたは遅延させる
  if (consecutiveFailures >= MAX_CONSECUTIVE_FAILURES) {
    if (consecutiveFailures === MAX_CONSECUTIVE_FAILURES) {
      const msg = `⚠️ システム警告: F2Pool への接続が ${MAX_CONSECUTIVE_FAILURES} 回連続で失敗しました。APIキーの有効期限またはURL仕様の変更を確認してください。自律ループを一時停止します。`;
      await postMessage('system', 'System Monitor', msg);
    }
    consecutiveFailures++; 
    // さらに 100回失敗したらリセットして再試行（念のため）
    if (consecutiveFailures > 105) consecutiveFailures = 0;
    return;
  }
  ...
  try {
    ...
    // 3. F2Poolから統計情報を取得
    const stats = await F2PoolBridge.fetchStats(currency, accountName, apiKey).then(s => {
      consecutiveFailures = 0; // 成功したらリセット
      return s;
    }).catch((err) => {
      consecutiveFailures++;
      console.warn(`⚠️ Orchestrator: Fetch failed (${consecutiveFailures}/${MAX_CONSECUTIVE_FAILURES}) for ${accountName} (${currency}):`, err.message);
      return {
        hashrate: 0,
        value_24h: 0,
        total_paid: 0,
