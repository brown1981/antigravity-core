...
export default {
  ...
  async fetch(request: Request, env: any, ctx: any) {
    // 💉 グローバル注入 (Failsafe Secret Injection)
    // Next.js の環境変数制限を回避するため、起動時に globalThis へ直接セットします。
    if (env.ENCRYPTION_MASTER_KEY) (globalThis as any).ENCRYPTION_MASTER_KEY = env.ENCRYPTION_MASTER_KEY;
    if (env.SUPABASE_SERVICE_ROLE_KEY) (globalThis as any).SUPABASE_SERVICE_ROLE_KEY = env.SUPABASE_SERVICE_ROLE_KEY;
    if (env.SUPABASE_URL) (globalThis as any).SUPABASE_URL = env.SUPABASE_URL;
